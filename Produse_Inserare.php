<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        {
            // insert
            if ($stmt = $mysqli->prepare("INSERT into tbl_product (name, code, image, price, descriere,
            categorie) VALUES (?, ?, ?, ?, ?, ?)"))
            {
                $stmt->bind_param("sssdss", $nume, $code,$file,$price,$descriere,$categorie);
                $stmt->execute();
                $stmt->close();
            }
            // eroare le inserare
...
<body>
    ...
    <form action="" method="post">
        <div>
            ...
            <a href="Produse_Vizualizare.php">Index</a>
        </div>
    </form>
</body>
</html>