    ...
    { 
        ...
        { 
            ...
            // preluam variabilele din URL/form
                ...
                if ($nume == '' || $username == ''||$parola==''||$email==''||$strada==''||$oras=='' || $tara=='' || $codPostal=='' || $nrCard=='' || $tipCard=='' || $dataExpCard=='' || $acceptareEmail=='' || $nrInregRC=='' || $codFiscal=='')
                ...
                { 
                    ...
                    if ($stmt = $mysqli->prepare("UPDATE clienti SET username=?, Parola=?, email=?, Strada=?, Oras=?, Tara=?, CodPostal=?, NrCard=?, TipCard=?, DataExpCard=?, AcceptareEmail=?, Nume=?, NrInregRC=?, cod_fiscal=? WHERE Client_id='".$id."'"))
                    {
                        $stmt->bind_param("ssssssssssssss", $username, $parola, $email, $strada, $oras, $tara, $codPostal, $nrCard, $tipCard, $dataExpCard,$acceptareEmail,$nume,$nrInregRC,$codFiscal);
                        $stmt->execute();
                        $stmt->close();
                    }
